...
public static class Animations
{
    ...
    public static void FadeIn(FrameworkElement element!!, double toValue = 1, int duration = 350) => AnimateDouble(element, UIElement.OpacityProperty, element.Opacity, toValue, duration);

    public static void FadeOut(FrameworkElement element!!, double toValue = 0, int duration = 350) => AnimateDouble(element, UIElement.OpacityProperty, element.Opacity, toValue, duration);
    ...
    public static void AnimateDouble(FrameworkElement element!!, string propertyPath, double from, double to, int duration = 350)
    {
        if (propertyPath.IsNullOrEmpty())
        {
            throw new ArgumentException($"'{nameof(propertyPath)}' cannot be null or empty", nameof(propertyPath));
        }
