using TypeData = Library.CodeGeneration.TypePath.TypeData;
...
public class TypePathTestData
{
    public static IEnumerable<object[]> ParseData
    {
        get
        {
            ...
            yield return new object[]
            {
                typeof(Task<IEnumerable<int?>>).FullName!, new TypeData(nameof(Task), typeof(Task).Namespace, [new(nameof(IEnumerable<int>), typeof(IEnumerable<>).Namespace, [new(nameof(Int32), typeof(int).Namespace, [], true)], false)], false)
            };
        }
    }
}