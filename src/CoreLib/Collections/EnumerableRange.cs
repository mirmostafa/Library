...
public sealed class Enumerator<TItem>(Func<TItem, (bool isOk, TItem next)> tryGetNext, Func<TItem>? getInitialValue = default, Action? onDispose = null) : IEnumerator<TItem>
{
    ...
    public bool MoveNext()
    {
        ...
        {
            (isOk, var next) = tryGetNext(this._current);
            if (isOk)
            {
                this._current = next;
            }
...
{
    public static Enumerable<TItem> New(Func<TItem, (bool isOk, TItem next)> tryGetNext, Func<TItem>? getInitialValue = default, Action? onDispose = null)
        => new(new Enumerator<TItem>(tryGetNext, getInitialValue, onDispose));
