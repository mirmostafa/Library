...
public static class ServiceHelper
{
                ...
                where TViewModel : IHasKey<long?>
    {
        ...
        if (!Check.TryIfArgumentIsNull(model?.Id, out var res1))
        {
            return res1;
        }
        if (!Check.TryIfArgumentIsNull(dbContext, out var res2))
        {
            return res2;
        }
    ...
    /// <summary>
    /// Asynchronously inserts a new entity into the database using the given service, view model, and database entity.
    /// </summary>
    /// <typeparam name="TService">The type of the service.</typeparam>
    /// <typeparam name="TViewModel">The type of the view model.</typeparam>
    /// <typeparam name="TDbEntity">The type of the database entity.</typeparam>
                ...
                where TId : notnull
    {
        ...
        if (Check.TryIfArgumentIsNull(model, out var res1))
        {
            return getResult(res1);
        }
        if (Check.TryIfArgumentIsNull(manipulate, out var res2))
        {
            return getResult(res2);
        }
        if (Check.TryIfArgumentIsNull(convertToEntity, out var res3))
        {
    ...
    /// <param name="persist">if set to <c>true</c> [persist].</param>
    /// <returns>
    /// A <see cref="TaskResultTViewModel"/> representing the asynchronous operation.
    /// </returns>
    /// <remarks>
