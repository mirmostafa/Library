...
namespace Mohammad.Data.SqlServer.Dynamics
{
    public class StoredProcedure : SqlObject<StoredProcedure, Database>
    {
        ...
        public StoredProcedureParams Params
        {
            get
            {
                return this._Params ?? (this._Params = new StoredProcedureParams(from row in (from row in this.Owner.AllParams
                                                                                              where row.Field<string>("SpName").EqualsTo(this.Name)
                                                                                              select row).ToList()
                                                                                 select
                                                                                 new StoredProcedureParam(this, row.Field<string>("name"), this.ConnectionString)
                                                                                 {
                                                                                     DefaultValue
                                                                                         =
                                                                                         row
        ...
        public StoredProcedure(Database owner, string name, string connectionstring)
            : base(owner, name, connectionstring) {}

        public bool Run(params KeyValuePair<string, object>[] args) { return true; }
