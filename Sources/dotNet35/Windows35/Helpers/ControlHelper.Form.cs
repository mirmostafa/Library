	...
	public static class ControlHelper
	{
		...
		public static TForm ShowSingleton<TForm>(Func<TForm> creator, FormSettings<TForm> formSettings, Predicate<TForm> predicate) where TForm : Form
		{
			var form = predicate == null
				? Application.OpenForms.Cast<Form>().Where(Library35.Helpers.Win.ControlHelper.EqualsTo<TForm>).FirstOrDefault() as TForm
				: Application.OpenForms.Cast<Form>().Where(frm => Library35.Helpers.Win.ControlHelper.EqualsTo<TForm>(frm) && predicate(frm as TForm)).FirstOrDefault() as TForm;
			if (form == null)
