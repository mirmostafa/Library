...
namespace Library35.Data.Ado
{
	public class SqlHelper
	{
		...
		public DataSet FillByQuery(string query)
		{
			...
			var result = builder.InitialCatalog.IsNullOrEmpty() ? new DataSet() : new DataSet(builder.InitialCatalog);
			using (var da = new SqlDataAdapter(query, this.ConnectionString))
				da.Fill(result);
			return result;
		}

		public SqlCommand GetCommand(string sqlStatement)
		{
			var connection = new SqlConnection(this.ConnectionString);
			var result = new SqlCommand(sqlStatement, connection);
		...
		public void FillDataTable(string sql, DataTable result, Action<SqlParameterCollection> fillParams)
		{
		...
		public DataTable FillDataTable(string sql, Action<SqlParameterCollection> fillParams)
		{
		...
		public int ExecuteNonQuery(string sql, Action<SqlParameterCollection> fillParams)
		{
			var result = 0;
		...
		public void TransactionalCommand(string sql, Action<SqlCommand> executer)
		{
			this.TransactionalCommand(sql, null, executer);
		}

		public void TransactionalCommand(string sql, Action<SqlParameterCollection> fillParams, Action<SqlCommand> executer)
		{
			if (sql == null)
		...
		private void TransactionalCommand(IEnumerable<CmdMgr> commandMgrs)
		{
			using (var connection = new SqlConnection(this.ConnectionString))
			{
				connection.Open();
		...
		public IEnumerable<object> ExecuteScalar(IDictionary<String, Action<SqlParameterCollection>> fills)
		{
