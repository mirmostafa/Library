...
using Library35.Validation.Attributes;
...
namespace Library35.Validation
{
	public class Validator : IExceptionHandlerContainer<Exception>
	{
		...
		/// <returns> </returns>
		protected bool ParseValidationByAttributes(object obj, PropertyInfo property, out ValidationException exception)
		{
			exception = null;
		...
		protected virtual void Throw<TValidationException>(Func<TValidationException> ctor) where TValidationException : ValidationException
		{
			if (this.ExceptionHandling != null)
				this.ExceptionHandling.HandleException(ctor());
			else
				CompanyException.Throw(ctor);
		}
	}
}