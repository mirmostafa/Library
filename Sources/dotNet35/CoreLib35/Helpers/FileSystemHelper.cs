...
namespace Library35.Helpers
{
	...
	public static class FileSystemHelper
	{
		...
		/// </param>
		public static void SafeDelete(this IEnumerable<FileInfo> files, EventHandler<ItemDeletingEventArgs<FileInfo>> onFileDeleting = null, bool sendToRecycleBin = false)
		{
		...
		///     if set to <c>true</c> sends to recycle bin.
		/// </param>
		public static void SafeDelete(this FileInfo file, EventHandler<ItemDeletingEventArgs<FileInfo>> onFileDeleting = null, bool sendToRecycleBin = false)
		{
			if (onFileDeleting != null)
			{
				var e = new ItemDeletingEventArgs<FileInfo>(file);
		...
		public static void SafeDelete(this IEnumerable<DirectoryInfo> directories,
			EventHandler<ItemDeletingEventArgs<DirectoryInfo>> onDirectoryDeleting = null,
			EventHandler<ItemDeletingEventArgs<FileInfo>> onFileDeleting = null,
		...
		public static void SafeDelete(this FileSystemInfo fileSystem,
			EventHandler<ItemDeletingEventArgs<FileSystemInfo>> onFileSystemDeleting = null,
			EventHandler<ItemDeletingEventArgs<FileInfo>> onFileDeleting = null,
			bool sendToRecycleBin = false)
		{
			if (IsDirectory(fileSystem.FullName))
			{
				var onDirectoryDeleting = new EventHandler<ItemDeletingEventArgs<DirectoryInfo>>(delegate(object sender, ItemDeletingEventArgs<DirectoryInfo> e)
				                                                                                 {
					                                                                                 if (e == null)
						                                                                                 return;
					                                                                                 var args = new ItemDeletingEventArgs<FileSystemInfo>(e.Item);
					                                                                                 if (onFileSystemDeleting != null)
						                                                                                 onFileSystemDeleting(sender, args);
					                                                                                 e.Canceled = args.Canceled;
				                                                                                 });
				fileSystem.As<DirectoryInfo>().SafeDelete(onDirectoryDeleting, onFileDeleting, sendToRecycleBin);
			}
			else
			{
				var handler = new EventHandler<ItemDeletingEventArgs<FileInfo>>(delegate(object sender, ItemDeletingEventArgs<FileInfo> e)
				                                                                {
					                                                                if (e == null)
						                                                                return;
					                                                                var args = new ItemDeletingEventArgs<FileSystemInfo>(e.Item);
			...
			bool sendToRecycleBin = false)
		{
			...
			foreach (var info in directory.GetFiles())
			{
				if (onFileDeleting != null)
				{
					var e = new ItemDeletingEventArgs<FileInfo>(info);
					onFileDeleting(null, e);
					if (e.Canceled)
						continue;
				}
			...
			foreach (var info2 in directory.GetDirectories())
			{
				...
				{
					var args2 = new ItemDeletingEventArgs<DirectoryInfo>(info2);
