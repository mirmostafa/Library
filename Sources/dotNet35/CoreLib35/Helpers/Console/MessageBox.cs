...
namespace Library35.Helpers.Console
{
	public static class MessageBox
	{
		...
		public static void WriteInBox(this object obj, object title)
		{
			WriteInBox(obj,
				title,
		...
		public static void WriteInBox(this object obj, object title, int width)
		{
			Open(width);
			string.Concat("*", title).Write();
			InsertSeparatorLine(title);
		...
		public static void WriteInBox(this object obj)
		{
			if (obj == null)
				return;
		...
		{
			WriteInBox(string.Format("[{0}] {1}", PersianDateTime.Now, text));
		}
	}
}