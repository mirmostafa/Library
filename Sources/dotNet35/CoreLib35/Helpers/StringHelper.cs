	...
	public static class StringHelper
	{
		...
		public static string GetPhrase(this string str, char start, char end, int index)
		{
			...
			for (var counter = 0; counter <= index; counter++)
			{
				...
				result = result.Remove(result.IndexOf(end));
				str = str.Substring(str.IndexOf(result) + result.Length + 1);
			}
			return result;
		}
		...
		public static string SeparateCamelCase(this string name)
		{
			...
			foreach (var currChar in name)
			{
				if (!char.IsDigit(currChar) &&
				    ((String.Compare(currChar.ToString(), "_", StringComparison.Ordinal) == 0) ||
				     (String.Compare(currChar.ToString(), currChar.ToString().ToUpper(), StringComparison.Ordinal) == 0)))
					parserResult.Append(' ');
