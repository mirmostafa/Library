    ...
    public class Database : SqlObject<Database, Server>
    {
        ...
        private static IEnumerable<Database> GatherDatabases(string connectionstring, string databases)
        {
            ...
            foreach (var row in GetDataRows(connectionstring, databases))
            {
                ...
                {
                    CollationName = row.Field<string>("collation_name"), CreateDate = row.Field("create_date", Convert.ToDateTime),
                    Id = row.Field("database_id", Convert.ToInt64), IsBrokerEnabled = row.Field("is_broker_enabled", Convert.ToBoolean)
                };
            }
        }
        ...
        private StoredProcedures GetStoredProcedures() => new StoredProcedures(
            ...
            {
                AssemblyName = row.Field("AssemblyName", Convert.ToString), Id = row.Field("ID", Convert.ToInt64), CreateDate = row.Field("CreateDate", Convert.ToDateTime),
        ...
        private Tables GetTables()
        {
            ...
            return new Tables(tables.Select(t => new Table(this, t.name, t.schema, this.ConnectionString)
                {
                    CreateDate = Convert.ToDateTime(t.create_date), Id = Convert.ToInt64(t.object_id), ModifyDate = Convert.ToDateTime(t.modify_date)
                })
                .ToList());
