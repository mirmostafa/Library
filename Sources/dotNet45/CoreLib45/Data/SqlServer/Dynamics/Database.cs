...
namespace Mohammad.Data.SqlServer.Dynamics
{
    public class Database : SqlObject<Database, Server>
    {
        ...
        public static Databases GetDatabases(string datasource, string username, string password) => GetDatabasesCore(ConnectionStringBuilder.Build(datasource, username, password));

        protected static Databases GetDatabasesCore(string connectionstring, string databases = "sys.databases") => new Databases(() => GatherDatabases(connectionstring, databases));
        ...
        private StoredProcedures GetStoredProcedures()
        {
            return new StoredProcedures(
                                        ...
                                        {
                                            AssemblyName = row.Field("AssemblyName", Convert.ToString), Id = row.Field("ID", Convert.ToInt64)
                                          , CreateDate   = row.Field("CreateDate",   Convert.ToDateTime), Body = GetQueryItems(this.ConnectionString,
        ...
        {
            result = this.StoredProcedures[binder.Name].Run(args.Select((t, i) => new KeyValuePair<string, object>(binder.CallInfo.ArgumentNames[i], t))
                                                                .ToArray());
            return true;
        }
    }
