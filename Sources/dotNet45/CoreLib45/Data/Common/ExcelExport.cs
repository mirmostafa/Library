...
namespace Mohammad.Data.Common
{
    public class ExcelExport : Singleton<ExcelExport>
    {
        ...
        public int RowLimit { get; set; } = 65000;
        ...
        /// <returns> </returns>
        public IEnumerable<FileInfo> Export(DataTableCollection datatables, string filename = null) => from DataTable table in datatables
                                                                                                       select this.Export(table);
        ...
        {
            var excelXml = this.GetExcelXml(dataSet);
        ...
        /// <param name="table"> The table. </param>
        /// <returns> </returns>
        public string GetExcelXml(DataTable table) => this.GetExcelXml(new[] {table});
        ...
        /// <param name="tables"> The tables. </param>
        /// <returns> </returns>
        public string GetExcelXml(IEnumerable<DataTable> tables) => this.GetExcelXml(tables.ToArray());
        ...
        /// <param name="tables"> The tables. </param>
        /// <returns> </returns>
        public IEnumerable<string> GetExcelXml(DataTableCollection tables) => from DataTable table in tables
                                                                              select this.GetExcelXml(table);
        ...
        /// <param name="dataSet"> The data set. </param>
        /// <returns> </returns>
        public string GetExcelXml(DataSet dataSet) => this.GetXmlCore(dataSet);
        ...
        protected virtual string GetWorksheets(DataSet source)
        {
            ...
            Func<Type, object, string> getCell = delegate(Type type, object cellData)
            {
                ...
                {
                    return $"<Cell><Data ss:Type=\"Number\">{data}</Data></Cell>{Environment.NewLine}";
                }

                if (type.Name.Contains("Date") && data.ToString() != string.Empty)
                {
                    return $"<Cell ss:StyleID=\"s63\"><Data ss:Type=\"String\">{this.FormatDateTime(data)}</Data></Cell>{Environment.NewLine}";
                }
                ...
                else
                {
                    ...
                    for (var i = 0; i < dt.Rows.Count; i++)
                    {
                        if (i % this.RowLimit == 0)
                        {
                            ...
                            sw.Write("\r\n<Worksheet ss:Name=\"" + replaceXmlChar(validateSheetName(dt)) +
                                     (i / this.RowLimit == 0 ? "" : Convert.ToString(i / this.RowLimit)) + "\">\r\n<Table>");
        ...
        {
            this.FormattingDateTime?.Invoke(this, e);
            return e;
        }
