...
namespace Mohammad.Helpers
{
    ...
    {
        public static void Raise(this Delegate handler, object sender, EventArgs e = null)
        {
            handler?.DynamicInvoke(sender, e ?? EventArgs.Empty);
        }
        ...
        public static Task RaiseAsync(this Delegate handler, object sender, TaskScheduler scheduler = null, Task task = null)
        {
            if (handler == null)
                return null;
            if (scheduler == null)
                return task?.ContinueWith(t => handler.DynamicInvoke(sender, EventArgs.Empty)) ??
                       Task.Run(() => handler.DynamicInvoke(sender,          EventArgs.Empty));
        ...
        public static Task<TEvenetArgs> RaiseAsync<TEvenetArgs>(this EventHandler<TEvenetArgs> handler, object sender, TEvenetArgs e,
                                                                TaskScheduler                  scheduler)
            => handler != null ? RaiseAsync(handler, sender, e, null, scheduler) : null;

        public static Task<TEventArgs> RaiseAsync<TEventArgs>(this EventHandler<TEventArgs> handler, object sender, TEventArgs e) => RaiseAsync(handler,
                                                                                                                                                sender,
                                                                                                                                                e,
                                                                                                                                                null,
                                                                                                                                                null);
