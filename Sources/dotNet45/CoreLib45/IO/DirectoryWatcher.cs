    ...
    public class DirectoryWatcher
    {
        ...
        private void Watch()
        {
            ...
            try
            {
                ...
                uint bytesReturned;
                while (this._EnableRaisingEvents && Api.ReadDirectoryChangesW(hDir,
                    pBuf,
                    bufsize,
                    this.WatchSubtree,
                    WindowsConstants.FILE_NOTIFY_CHANGE_FILE_NAME |
                    WindowsConstants.FILE_NOTIFY_CHANGE_DIR_NAME |
                    ...
                    IntPtr.Zero))
                {
                    ...
                    switch (action)
                    {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            this.OnRenamed(new ChangedEventArgs<string>(System.IO.Path.Combine(this.Path.FullName, file),
                                System.IO.Path.GetFileName(newNameFile)));
                            break;
                    }
                }
