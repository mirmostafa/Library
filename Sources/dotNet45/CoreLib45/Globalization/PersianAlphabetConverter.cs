...
namespace Mohammad.Globalization
{
    public class PersianAlphabetConverter
    {
        ...
        {
            var pageCode1256 = Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(1256), Encoding.UTF8.GetBytes(input))
                                       .Where(b => !GetIgnoreCharacters().Contains(b)).ToList();
        ...
        private class PersianCharacter
        {
            ...
            public PersianCharacter(byte pC1256Code, byte isolatedCode, byte beginCode, byte middleCode, byte endCode, bool isStickyToBegin,
                                    bool isStickyToEnd,
