...
namespace Mohammad.ServiceProcess
{
    public class WindowsServiceController
    {
        ...
        public IEnumerable<WindowsServiceContriller> DependentServices => this._ServiceController.DependentServices.Select(
            ds => new WindowsServiceContriller(ds));

        public string ServiceName { get; internal set; }

        public IEnumerable<WindowsServiceContriller> ServicesDependedOn => this._ServiceController.ServicesDependedOn.Select(
            ds => new WindowsServiceContriller(ds));
        ...
        public static IEnumerable<WindowsServiceContriller> GetServices()
        {
            return ServiceController.GetServices().Select(sc => new WindowsServiceContriller(sc));
        }

        public static IEnumerable<WindowsServiceContriller> GetServicesByDisplayName(string displayName) => ServiceController.GetServices()
            .Where(sc => sc.DisplayName == displayName).Select(sc => new WindowsServiceContriller(sc));
