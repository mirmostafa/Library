...
namespace Mohammad.Wmi
{
    public class WmiTools
    {
        ...
        private static IEnumerable<TWmiQueryClass> Query<TWmiQueryClass>(ManagementScope scope)
            where TWmiQueryClass : WmiBase, new() => Query<TWmiQueryClass>(scope, GetWmiClassName<TWmiQueryClass>(), GetProperties<TWmiQueryClass>());

        private static IEnumerable<TWmiQueryClass> Query<TWmiQueryClass>(ManagementScope                           scope, string wmiClassName,
        ...
        private static IEnumerable<KeyValuePair<string, string>> GetProperties<TWmiQueryClass>()
            where TWmiQueryClass : WmiBase, new() => typeof(TWmiQueryClass)
                                                    .GetProperties().Where(p => ObjectHelper.GetAttribute<WmiPropAttribute>(p) != null)
                                                    .Select(p => new KeyValuePair<string, string>(
                                                                p.Name, ObjectHelper.GetAttribute<WmiPropAttribute>(p).Name ?? p.Name));
