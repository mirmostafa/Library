namespace Mohammad.Web.Asp.UI
{
    public abstract class LibraryBasePage : BasePage
    {
        ...
        {
            this.ShowMessage(message, title ?? this.Title, "info | ok");
        }

        protected void ShowError(string message, string title = null)
        {
            this.ShowMessage(message, title ?? this.Title, "warn | ok");
        }

        protected void ShowWarning(string message, string title = null)
        {
            this.ShowMessage(message, title ?? this.Title, "error | ok");
        }

        protected void ShowQuestion(string message, string title = null)
        {
            this.ShowMessage(message, title ?? this.Title, "question | yes | no");
        }
    }
}