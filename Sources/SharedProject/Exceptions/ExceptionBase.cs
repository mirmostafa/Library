...
namespace Mohammad.Exceptions
{
    ...
    public abstract class ExceptionBase : Exception, IException
    {
        ...
        protected ExceptionBase(string message, string instruction)
            : this(message) => this.Instruction = instruction;
        ...
        public static void WrapThrow<TException>(string message, string instruction, object owner)
            where TException : ExceptionBase
        {
            WrapThrow<TException>(message, instruction, null, owner);
        }

        public static void WrapThrow<TException>(string message, string instruction, IException ex)
            where TException : ExceptionBase
        {
            WrapThrow<TException>(message, instruction, ex, null);
        }

        public static void WrapThrow<TException>(string message, string instruction, IException ex, object owner)
            where TException : ExceptionBase
        {
        ...
        public static void WrapThrow<TException>(string message, string instruction, Exception ex)
            where TException : ExceptionBase
        {
            var me = ObjectHelper.CreateInstance<TException>(new[] {typeof(string), typeof(Exception), typeof(string)},
                new object[] {message, ex, instruction});
