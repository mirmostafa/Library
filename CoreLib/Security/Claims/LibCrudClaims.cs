...
public static class LibCrudPolicies
{
    ...
    public static (string Name, Action<AuthorizationPolicyBuilder> ConfigurePolicy) AdminOrFullAccessPolicy
    {
        get
        {
            void configurePolicy(AuthorizationPolicyBuilder policy) => policy.RequireAssertion(context => context.User.IsInRole("Admin") || context.User.IsInRole("SuperAdmin") || LibCrudClaims.GetClaimTypes().All(x => context.User.HasClaim(x, LibCrudClaims.VALID_CLAIM_VALUE)));
